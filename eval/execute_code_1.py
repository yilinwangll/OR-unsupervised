...
def extract_from_code(code):
    """从代码中提取 python 代码块的内容，支持多种格式"""
    if not code:
        return ""
    
    # 1. 尝试提取 ```python ... ``` 格式
    python_match = re.search(r'```python\s*(.*?)\s*```', code, re.DOTALL | re.IGNORECASE)
    if python_match:
    ...
    # 3. 尝试提取 <code> ... </code> 格式
    tag_match = re.search(r'<code>(.*?)</code>', code, re.DOTALL | re.IGNORECASE)
    if tag_match:
...
def compile_script(script_content, timeout=300):
    ...
    try:
        ...
        # 检查是否有执行错误（Traceback, Error, Exception等）
        error_patterns = [
        ...
        if has_error:
            ...
            return {
                ...
                "execution_best_solution": None,
                "execution_state": f"Execution Failed: {error_type}"
            }
...
"""
python /home/work/mllm_datas/yilin/code/OR-SR1/datasets/eval/execute_code_1.py \
